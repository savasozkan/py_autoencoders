...
class contractive_autoencoder(base_autoencoder):
    ...
    def loss(self, X, reg=0.001): 
        ...
        loss_jacobian = np.reshape(hid_act*(1.0-hid_act), (ntrain, 1, nhidden))*np.reshape(W, (1, ninput, nhidden))
        ...
        # This part 
        err_hid_act = err_W1*self.ac_func_deriv(hid_act, self.act_func) + reg*(hid_act*(1.0-hid_act)*(1-2.0*hid_act))*np.sum(W*W, axis=0)/ntrain
